...
def get_network(name, batch_size, layout="NHWC", dtype="float32", use_sparse=False):
    ...
    desired_layouts = {'nn.conv2d': ['NHWC', 'default'], 'image.resize2d': ['NHWC', 'default'], 'nn.upsampling': ['NHWC', 'default']}
...
def run_tuning():
    ...
    tune_option = auto_scheduler.TuningOptions(
        num_measure_trials=6400,  # change this to 20000 to achieve the best performance
        builder = auto_scheduler.LocalBuilder(timeout=1000),
        #  runner=measure_ctx.runner,
        runner = auto_scheduler.RPCRunner('nano', '10.36.172.151', 9190, min_repeat_ms=300, timeout=1000, repeat=1),
